...
namespace Photon.Pun.UtilityScripts
{
    public class ConnectAndJoinRandom : MonoBehaviourPunCallbacks
    {
        ...
        public string roomId; // B?n có th? thay ??i thành "2" n?u mu?n tham gia phòng 2
        ...
        private void JoinSpecificRoom()
        {
            ...
            PhotonNetwork.JoinRoom(roomId); // Tham gia phòng theo ID ?ã ch? ??nh
        }

        public override void OnJoinRoomFailed(short returnCode, string message)
        {
            Debug.Log("OnJoinRoomFailed() was called by PUN. Room not found, creating room: " + roomId);
            RoomOptions roomOptions = new RoomOptions() { MaxPlayers = this.MaxPlayers };
            if (playerTTL >= 0)
                roomOptions.PlayerTtl = playerTTL;

            PhotonNetwork.CreateRoom(roomId, roomOptions, null); // T?o phòng m?i n?u không tìm th?y
        }
